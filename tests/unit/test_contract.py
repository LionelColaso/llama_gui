...
def test_cli_describe_json_envelope_matches_schema(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    # Guards against CLI/schema drift: the real `describe --json` output must
    # validate against Envelope *and* its nested DescribeData payload.
    import contextlib
    import io
    import json
    ...
    with contextlib.redirect_stdout(buf):
        rc = cli_main(["--root", str(tmp_path), "describe", "--json"])
    assert rc == 0

    payload: object = json.loads(buf.getvalue())
    # The envelope shape matches schemas.py exactly...
    env = Envelope.model_validate(payload)
    assert env.contract_version == contract_version
    assert env.action == "describe"
    assert env.ok is True
    # ...and the nested `data` matches DescribeData (catches schema drift).
    describe = DescribeData.model_validate(env.data)
    assert {b.name for b in describe.backends} >= {"vulkan", "cpu"}
    assert "describe" in describe.available_actions
