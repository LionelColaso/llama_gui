...
def _build_nuitka_command(product_version: str = "", dev: bool = False) -> list[str]:
    args = [
        ...
        # Nuitka's bundled clcache (its C compiler cache) intermittently fails
        # the preprocessor on MSVC for large frozen modules (e.g. pygments
        # lexers) with "clcache: preprocessor failed". Disable it so the build
        # is deterministic; on Linux/macOS Nuitka falls back to ccache when
        # present (provided by CI's ccache-action).
    ...
    if system == "Windows":
        ...
        # On macOS produce an .app bundle (adopted from RimDex's Nuitka layout)
